...
class tkvideo():
    ...
    """
    def __init__(self, path, label, loop = 0, size = (640,360), fps = 0):
        self.path = path
        self.label = label
        self.loop = loop
        self.size = size
    ...
    def load(self, path, label, loop, fps):
        """
    ...
    def play(self):
        """
            ...
            the video's frames.
        """
        thread = threading.Thread(target=self.load, args=(self.path, self.label, self.loop, self.fps))
        thread.daemon = 1
        thread.start()
